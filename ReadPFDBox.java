...
public class ReadPFDBox {
	...
	public void validatepdfvspdfCompareLine(TestCaseParam testCaseParam, String pdfSourceFile, String pdfTargetFile, String resultFile, String sourceTextFile, String targetTextFile, int count1) throws PDFComparisonException, InterruptedException {
	    try {
	        ...
	        if (totalpageofDocument1 == totalpageofDocument2) {
	            for (int i = 1; i < (totalpageofDocument1 - count1) + 1; i++) {
	                extracted(testCaseParam, resultFile, sourceTextFile, targetTextFile,  document2,
	...
	private void extracted(TestCaseParam testCaseParam, String resultFile, String sourceTextFile,
		...
		try(BufferedReader br=new BufferedReader(new FileReader(sourceTextFile+i+".txt",StandardCharsets.UTF_8)))
		{
		...
		Thread.sleep(6000);
		String testcontent1 = stringBuilder.toString().replace("\\s+", " ").replace("\\n", "").replace("\\t", "").replaceAll("[^A-Za-z0-9. ]+", "");
		testcontant2 = testcontant2.replaceAll("[^A-Za-z0-9. ]+", "");
		
		if(!testcontent1.equalsIgnoreCase(testcontant2)){

			String targetTxtFile=targetTextFile+"\\TargetTextFile"+i+".txt";
			
			sourceTextFile=sourceTextFile+"\\ReplacedTextFile"+i+".txt";
			List<String> linenumber = textdiffutil(sourceTextFile,targetTxtFile);
